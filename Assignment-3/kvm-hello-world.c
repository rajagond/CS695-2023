...
void handler_io_exit(struct vm *vm, struct vcpu *vcpu)
{
	char *chptr = (char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset;
	uint32_t *ptr = (uint32_t *)(chptr);
	uint32_t addr;
	char *str;
	...
	if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT)
	{
		...
		{
		case PRINT8BIT_PORT:
			fwrite(ptr, vcpu->kvm_run->io.size, 1, stdout);
			break;
		case PRINT32BIT_PORT:
			fprintf(stdout, "%u\n", ptr[0]);
			fflush(stdout);
			break;
		case STR_PORT:
			addr = *ptr;
			str = ((char *)vm->mem + addr);
			fprintf(stdout, "%s", str);
			fflush(stdout);
			// uint32_t *ptr2 = (uint32_t *)vcpu->kvm_run;
			// fprintf(stdout, "%s", vm->mem + ptr2[vcpu->kvm_run->io.data_offset / 4]);
			break;
		case GVA_TO_HVA_PORT:
			vcpu->gva = ptr[0];
			break;
		default:
	...
	{
		uint32_t *ptr = (uint32_t *)vcpu->kvm_run;
		switch (vcpu->kvm_run->io.port)
		{
		case NUM_EXITS_PORT:
			ptr[vcpu->kvm_run->io.data_offset / 4] = vcpu->numExit;
			break;
		case GET_TIMESTAMP_PORT:
			if (ioctl(vm->fd, KVM_GET_CLOCK, &clock_data) < 0)
			{
				perror("KVM_GET_CLOCK");
				exit(1);
			}
			ptr[vcpu->kvm_run->io.data_offset / 4] = (uint32_t)clock_data.clock;
			break;
		case GVA_TO_HVA_PORT:
			tr.linear_address = vcpu->gva;
			...
			if (tr.valid == 0)
			{
				...
				fflush(stdout);
				ptr[vcpu->kvm_run->io.data_offset / 4] = 0;
			}
			else
			{
				ptr[vcpu->kvm_run->io.data_offset / 4] = (uint32_t)(uintptr_t)(vm->mem + tr.physical_address);
			}
			break;
		default:
			fprintf(stderr, "Unknown IO port: %d\n", vcpu->kvm_run->io.port);
	...
	{
		fprintf(stderr, "Unknown IO direction %d \n", vcpu->kvm_run->io.direction);
		exit(1);
	}
}
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			handler_io_exit(vm, vcpu);
			continue;

		case KVM_EXIT_MMIO:
			fprintf(stderr, "Got KVM_EXIT_MMIO, should not happen\n");
			continue;

		case KVM_EXIT_SHUTDOWN:
			fprintf(stderr, "Got KVM_EXIT_SHUTDOWN, should not happen\n");
			exit(1);
			/* fall through */
		default:
